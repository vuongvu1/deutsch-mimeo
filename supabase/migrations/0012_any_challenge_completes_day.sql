-- Deutsch MiMeo: a day counts as complete once ANY active challenge hits its goal.
--
-- Run this in Supabase Studio → SQL Editor → New query.
--
-- Replaces the required/optional split from 0010: there are no optional
-- challenges any more, and all_complete is bool_or instead of bool_and.
...
select
  e.user_id,
  e.local_date,
  bool_or(coalesce(d.total_seconds, 0) >= e.daily_goal_seconds) as all_complete,
...
alter table challenges drop column if exists optional;
