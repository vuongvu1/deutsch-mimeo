...
const _TtsSession = class _TtsSession {
  ...
  async init() {
    const { createPiperPhonemize } = await import("./piperPhonemize.js");
  ...
  async predict(text, opts) {
    ...
    // [mimeo patch] Drop phoneme IDs the model's embedding can't handle.
    // The bundled eSpeak phonemizer emits the full 256-symbol IPA set, but
    // older Piper voices (e.g. de_DE-eva_k-x_low, de_DE-kerstin-low) only
    // know `num_symbols` entries (often 130). Out-of-range IDs make the
    // model's Gather op throw and the whole sentence silently drops; better
    // to skip those tokens and lose a bit of prosody than to lose the
    // sentence. See research notes 2026-05-25.
    const numSymbols = __privateGet(this, _modelConfig).num_symbols;
    const safePhonemeIds = typeof numSymbols === "number"
      ? phonemeIds.filter((id) => id >= 0 && id < numSymbols)
      : phonemeIds;
    if (safePhonemeIds.length === 0) {
      // Nothing left after filtering — return a 1-sample silent WAV so the
      // caller can still advance to the next sentence without crashing.
      const sr = __privateGet(this, _modelConfig).audio.sample_rate;
      return new Blob([pcm2wav(new Float32Array(1), 1, sr)], { type: "audio/x-wav" });
    }
    const speakerId = 0;
    ...
    const feeds = {
      input: new (__privateGet(this, _ort)).Tensor("int64", safePhonemeIds, [1, safePhonemeIds.length]),
      input_lengths: new (__privateGet(this, _ort)).Tensor("int64", [safePhonemeIds.length]),
...
async function voices() {
  try {
    ...
    const LOCAL_VOICES_JSON = await import("./voicesStatic.js");
